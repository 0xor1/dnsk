...
namespace Dnsk.Cli;
class StateFilter([FromServices] State state, [FromServices] IApi api, ConsoleAppFilter next) : ConsoleAppFilter(next)
{
    public override async Task InvokeAsync(ConsoleAppContext ctx, CancellationToken ctkn)
    {
        ...
        finally
        {
            ...
            var appDataDir = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData, Environment.SpecialFolderOption.Create);
